...
import { analyzeGame, getImpliedProbability, americanToDecimal } from '../services/predictionService';
import { TrendingUp, Trash2, PlusCircle, AlertTriangle } from 'lucide-react';
...
const Calculator: React.FC = () => {
    ...
    const [parlayOdds, setParlayOdds] = useState<number>(100);
    ...
    useEffect(() => {
        if (teamAId && teamBId && teamAId !== teamBId) {
            // Create a dummy game object for analysis
            const dummyGame: Game = {
                id: 'CALC_GAME',
                homeTeamId: teamAId,
                awayTeamId: teamBId,
                time: 'Custom',
                odds: { // These are just placeholders, not used in core calc
            ...
            const analysis = analyzeGame(dummyGame);
    ...
    const impliedProbability = getImpliedProbability(odds);
    const value = prediction ? prediction.winner.team.id === teamAId ? prediction.winner.probability - impliedProbability : (100 - prediction.winner.probability) - impliedProbability : 0;

    const handleAddLeg = () => {
        if(parlayDescription.trim() === '' || !parlayOdds) return;
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700">
                <h2 className="text-2xl font-bold mb-4 text-white">Probability Calculator</h2>
                ...
                <div className="space-y-3 mb-4">
                 ...
                 <div className="flex items-end gap-2 mb-4">
                    ...
                    <div>
                        ...
                        <input type="number" value={parlayOdds} onChange={e => setParlayOdds(parseInt(e.target.value))} className="w-24 bg-gray-700 text-white rounded-md p-2 border border-gray-600 focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm"/>
                    </div>
                 ...
                 <div>
                    ...
                    <input type="number" value={wager} onChange={e => setWager(parseFloat(e.target.value))} className="w-full bg-gray-700 text-white rounded-md p-2 border border-gray-600 focus:ring-2 focus:ring-blue-500 focus:outline-none"/>
                </div>
                 <div className="mt-4 bg-gray-900/50 p-4 rounded-lg border border-gray-700 text-center">
                    ...
                    <p className="text-3xl font-extrabold text-green-400">${parlayResult.payout.toFixed(2)}</p>
                </div>
